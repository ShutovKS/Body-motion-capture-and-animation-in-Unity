...
using UnityEditor;
using UnityEngine;
using EditorUtil = Example_2___IK_Animation.SAFullBodyIK.Editor.FullBodyIKEditorUtility;

namespace Example_2___IK_Animation.SAFullBodyIK.Editor
{
	public class FullBodyIKInspectorBase : UnityEditor.Editor
	{
		...
		void _BoneField( string boneName, ref FullBody.FullBodyIK.Bone bone, bool isOptional )
		{
			FullBody.FullBodyIK.SafeNew( ref bone );
		...
		void _FingerBoneField( string boneName, ref FullBody.FullBodyIK.Bone[] bones, bool isOptional )
		{
			if( bones == null || bones.Length != 4 ) {
				bones = new FullBody.FullBodyIK.Bone[4];
			}
		...
		void _EffectorField( string effectorName, ref FullBody.FullBodyIK.Effector effector )
		{
			var fbik = this.target as FullBodyIKBehaviourBase;
	...
	[CustomEditor( typeof( FullBodyIKBehaviour ) )]
